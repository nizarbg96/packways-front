...
export class StatClientComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getStatClient();
    const date = new Date(this.date.getTime() - (60 * 60 * 24 * 30 * 1000));
    this.getColisLivreeFromDate(date);
  }

  getStatClient() {
    this.statestiquesService.getClientStats(JSON.parse(localStorage.getItem('currentUser')).data[0].idUser).subscribe((res) => {
