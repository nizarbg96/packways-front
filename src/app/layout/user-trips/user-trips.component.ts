...
import { globalJsVar } from '../trips/tunisia.js';
import { globalJsVarUp } from '../trips/tunisia-adr.js';
...
const TunisiaGovAndDelg = require('./../trips/tunisia.js');
const TunisiaAdr = require('./../trips/tunisia-adr.js');
type AOA = any[][];
@Component({
...
export class UserTripsComponent implements OnInit {
  ...
  descriptionTrip: any;
  modeLiv = "24H";
  typePaiement = "Contre remboursement";
  sizePack: string;
  pricePack: number;
  dataUser: any;
  poidVol: any;
  ...
  statusList = ['cherche un livreur', 'Livreur en chemin', 'Chez Livreur', 'livraison en cours', 'Livree', 'Annulée',
    'Retour', 'En cours de retour', 'Retournee' ];
  ...
  disableInputValTrip = false;
  editAdrDestFlag = false;
  zipCodeUp: any;
  ...
  ngOnInit() {
    ...
    this.getFiltredTrips1(this.startDateFilter, this.endDateFilter, this.searchTerm, this.clientFilter2,
      this.stateFilter, this.enRetardFilter, this.payementStatusFilter, this.driverFilter2, null, this.adminFilterkey);
    // this.getTripsOnInit();
    ...
    if(this.Adresses.length > 0){
      this.selectedAdresseExp = this.Adresses[this.Adresses.length - 1];
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(UserDialogOverviewExampleDialog, {
      width: '60%',
  ...
  openAddModal(content) {
    ...
    const compoAdr = $( '#user-cityGlobalDest' );
    const zip = $( '#user-zipCode' );
    const delegation2 = $( '#user-delegation2' );
  ...
  getFiltredTrips1(dateTp, dateTp2, keyTp, keyTp1, keyTp2, keyTp3, keyTp4, keyTp5, btnTp, keyTp6) {
    ...
    if (this.auth === 'admin') {
      ...
      this.idadmin = this.dataUser.idAdmin;
      // console.log('idadmin== ' + this.idadmin);
      this.permiss = this.dataUser.financier;
    } else {
      this.id = this.currentUser.data[0].idUser;
    ...
    this.tservice.getFiltredTrips1(this.id, this.chooseView, this.dateFiltredTrip, this.dateFiltredTrip2,
      ...
      this.keyFiltredTrip5, this.btnFiltredTrip, this.adminFilterkey).subscribe(data => {
      this.result = data['_body'];
      const jo = JSON.parse(this.result);
      const obj = Array.of(jo.data);
      this.jsonObj = obj[0];
  ...
  getFiltredTripsOnPrevious() {
    if (this.items.length !== 0) {
      ...
      this.getFiltredTrips1((this.startDateFilter), prevTripDate, this.searchTerm, this.clientFilter2, this.driverFilter2,
        this.stateFilter, this.enRetardFilter, this.payementStatusFilter, btnPrev, this.adminFilterkey);
    } else {
      this.prvBtnDisabled = true;
      this.getFiltredTrips1((this.startDateFilter), (this.endDateFilter), this.searchTerm, this.clientFilter2,
        this.stateFilter, this.enRetardFilter, this.payementStatusFilter, this.driverFilter2, null, this.adminFilterkey);
  ...
  closeTrip() {
    const id = this.checkedTrips[0].userTrip.idUser;
    this.closedTrips = [];
    for (let i = 0; i < this.checkedTrips.length; i++) {
  ...
  PresrecolterTrip() {
    ...
    for (let i = 0; i < this.checkedTrips.length; i++) {
      ...
      if ((this.checkedTrips[i].paymentStatus === 'Récolte') || (this.checkedTrips[i].argentRecolte === true) || (this.checkedTrips[i].paymentStatus === 'En cours de payement') || (this.checkedTrips[i].paymentStatus === 'Payee') || (this.checkedTrips[i].isClosed === true) || (this.checkedTrips[i].preRecolte === true)) {
        this.snackBar.open('Immpossible de modifier le colis ' + this.checkedTrips[i].refTrip, 'Fermer', {
          duration: 12000,
        });
        return;
    ...
    console.log('fin updateprésrecolté:');
    // affection de livreur
  ...
  editTrip(content, trip) {
    ...
    $('#user-newAdrDest').hide();
    $('#user-newAdrSource').hide();
    const gouvernoratUp = $( '#user-gouvernoratUp' );
    const delegationUp = $( '#user-delegationUp' );
    const key = this.zipCodeUp;
    const compoAdr = $( '#user-cityGlobalDestUp' );
    const zip = $( '#user-zipCodeUp' );
  ...
  addTrip() {
    ...
    const a = $('#user-latGlobalDest').val();
    // const a = $('36.81897').val();
    const b = $('lngGlobalDest').val();
    // const b = $('10.16579').val();
    const c = $('#user-cityGlobalDest').val();

    newAdr = $('#user-cityGlobalDest').val();
    console.log('desttttttt', a, '1 + ', b, '2 +', c);
    contactTel = $('#user-inputDestTel').val();
    // const regex =/^[0-9]+$/;
    ...
    if (!this.contactAdresseDest) {
      ...
      return;
    } else if (this.cityAdresseDest === undefined) {
      this.snackBar.open('Veuillez remplir le champ adresse.', 'Fermer', {
        duration: 5000,
      });
      return;

    } else if (this.gouvernorat === undefined) {
      this.snackBar.open('Veuillez sélectionner le gouvernorat.', 'Fermer', {
        duration: 5000,
      });
      return;
      ...
      this.snackBar.open('Veuillez saisir la ville.', 'Fermer', {
        duration: 5000,
      });
      return;
    ...
    let g = $('#user-gouvernorat').val();
    let d = $('#user-delegation').val();
    const z = $('#user-zipCode').val();
    const adr = $('#user-inputDestAdresseCity').val();
    if ( g === '' ) {
      g = this.gouvernorat;
    }
    if (d === '') {
    ...
    // console.log((this.cityGlobalDest).indexOf('Gouvernorat'));
    ...
    if (this.selectedAdresseDest !== undefined) {
      ...
      // labelAdrD = (<HTMLInputElement>document.getElementById('user-inputDestAdresseCity')).value;
      labelAdrD = adr;
      console.log('2222');
    } else {
      labelAdrD = '';
    ...
    if (this.cityGlobalDest === '') {
      ...
      this.cityGlobalDest = '' + labelAdrD + ' ' + this.cityGlobalDest;
    }

    if (labelAdrD === '') {
      this.cityGlobalDest = this.gouvernorat ;
    }
    console.log('this.cityGlobalDest', this.cityGlobalDest);
    this.cityGlobalDest = this.cityAdresseDest + ' ' + this.gouvernorat + ' ' + this.delegation ;
    ...
    this.tservice.addTrip(
      ...
      null, this.selectedAdresseExp.labelAdr, null, this.gouvernorat, this.delegation, this.zipCode, this.cityAdresseDest);
  ...
  updatetrip() {
    const ac = $('#user-cityAdresseDestUp').val();
    const g = $('#user-gouvernoratUp').val();
    const d = $('#user-delegationUp').val();
    const z = $('#user-zipCodeUp').val();
    let adr: any; let newLat: any; let newLng: any; let oldAdr: any; let newAddress: any;
    adr = $('#user-cityGlobalDestUp').val();
    oldAdr = $('#user-cityAdrUp').val();
    newLat = $('#user-latGlobalDestUp').val();
    newLng = $('#user-lngGlobalDestUp').val();
    const titleBtn = $('#user-btnShowHide').text();
    newAddress = ac + ' ' + adr;
    adr = newAddress;
  ...
  showResultV2(result) {
    $( '#user-latGlobalDestUp' ).val(result.geometry.location.lat());
    $( '#user-lngGlobalDestUp' ).val(result.geometry.location.lng());
  ...
  actionEtat() {
    ...
    if (this.itemStatus === 'EntrepotTunis') {
      this.itemStatus = 'Chez Livreur';
      driverAffect = 'EntrepotTunis';
    } else if (this.itemStatus === 'EntrepotSahel') {
      this.itemStatus = 'Chez Livreur';
      driverAffect = 'EntrepotSahel';
    } else if (this.itemStatus === 'EntrepotSfax') {
      this.itemStatus = 'Chez Livreur';
    ...
    /*
        if (this.itemStatus === 'Chez Livreur') {
          for (let i = 0; i < this.checkedTrips.length; i++) {
  ...
  getAllDrivers() {
    this.tservice.getDrivers().subscribe(data => {
      ...
      for (let i = 0; i < this.jsonObj.length; i++) {
        if ((this.jsonObj[i].accountActive === true) && (this.jsonObj[i].idDriver !== '5ca28097e4970623916b53e7')) {
  ...
  getAllAdmins() {
    this.tservice.getAdmins().subscribe(data => {
      this.result = data['_body'];
      // console.log(data['_body'])
      const jo = JSON.parse(this.result);
      const obj = Array.of(jo.data);
      ...
      // this.Listuser = this.jsonObj;
      for (let i = 0; i < this.jsonObj.length; i++) {
        this.Listadmin.push(this.jsonObj[i]);
        this.Listadminauto.push(this.jsonObj[i].name);
      }
      // console.log('listuser!::', this.Listuserauto);
    }, error => {
      // console.log(error);
  ...
  show1() {
    document.getElementById('user-div_dimensions').style.display = 'none';
    console.log('none');
    ...
    const address = (<HTMLInputElement>document.getElementById('user-cityGlobalDest')).value;
    // $( '#cityGlobalDest' ).val(address);
    // this.getLatitudeLongitudeFromAddress(this.showResult, address);
  }
  show2() {
    document.getElementById('user-div_dimensions').style.display = 'block';
    console.log('block');
    this.distancePack = this.calculateDistance(this.selectedAdresseExp.geolocAdr.lat, this.selectedAdresseExp.geolocAdr.lng,
      this.latGlobalDest, this.lngGlobalDest, 'K' );
    console.log('distancePack', this.distancePack);
  ...
  onClickAdrDest() {
    if (!this.selectedAdresseExp) {
      ...
      if (this.selectedAdresseDest.adresse === '' || this.selectedAdresseDest.adresse === null) {
        this.cityAdresseDest = this.selectedAdresseDest.cityAdr;
      } else {
        this.cityAdresseDest = this.selectedAdresseDest.adresse;
        this.gouvernorat = this.selectedAdresseDest.gouvernorat;
        this.delegation = this.selectedAdresseDest.ville;
        this.zipCode = this.selectedAdresseDest.zipCode;
      }
      this.contactAdresseDest = this.selectedAdresseDest.contactAdr;
      this.telContAdresseDest = this.selectedAdresseDest.mobileAdr;

      $('#user-latGlobalDest').val(this.selectedAdresseDest.geolocAdr.lat);
      $('#user-lngGlobalDest').val(this.selectedAdresseDest.geolocAdr.lng);
      $('#user-cityGlobalDest').val(this.selectedAdresseDest.cityAdr);
  ...
  deleteTrip(trip) {
    if (localStorage.getItem('auth') === 'admin') {
      ...
      $('#user-trip-row-' + trip.idTrip).hide('slow', function() {
        $(this).remove();
      });
  ...
  setCheckedTrip() {
    if (this.checkedTrips != null) {
      for (let i = 0; i < this.checkedTrips.length - 1; i++) {
        ...
        $('#user-checked-trip-' + sid).attr( 'checked' );
      }
  ...
  public generateDechargeForAdmin() {
    const data = document.getElementById('user-contentToConvert');
    const dj = new Date();
    ...
    html2canvas(data).then(canvas => {
      ...
      for (let i = 0; i < this.checkedTrips.length; i++) {
        const sid = this.checkedTrips[i].idTrip;
        $('#user-checked-trip-' + sid).prop('checked', false);
      }
      this.checkedTrips = [];
      this.dateJ = '';
  ...
  public generateBLivraison() {
    ...
    const data = document.getElementById('user-contentBlToConvert');
  ...
  generateDechargeForClient() {
    const doc = new jsPDF();
    doc.autoTable({html: '#user-contentToConvert'});
  ...
  searchPlaces() {
    ...
    this.input = document.getElementById('user-inputDestAdresseCity');
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      $('#user-latGlobalDest').val(place.geometry.location.lat());
      $('#user-lngGlobalDest').val(place.geometry.location.lng());
      $('#user-cityGlobalDest').val(address);
    });
  ...
  searchPlacesOnUpTrip() {
    ...
    this.input = document.getElementById('user-cityAdresseDestUp');
    const autocomplete = new google.maps.places.Autocomplete(this.input as HTMLInputElement, options);
    autocomplete.addListener('place_changed', () => {
      const place: google.maps.places.PlaceResult = autocomplete.getPlace();
      if (place.geometry === undefined || place.geometry === null) {
      ...
      $('#user-latGlobalDestUp').val(place.geometry.location.lat());
      $('#user-lngGlobalDestUp').val(place.geometry.location.lng());
      $('#user-cityGlobalDestUp').val(address);
    });

    this.latGlobalDest = $('#user-latGlobalDestUp').val();
    this.lngGlobalDest = $('#user-lngGlobalDestUp').val();
  ...
  searchPlacesV2() {
    ...
    searchBox.addListener('places_changed', function() {
      ...
      $('#user-latGlobalDest').val(places[0].geometry.location.lat());
      $('#user-lngGlobalDest').val(places[0].geometry.location.lng());
      $('#user-cityGlobalDest').val(address);
      console.log(address);
  ...
  getDelegFromGov() {
    const gov = $( '#user-gouvernorat' ).val();
    const zp = $( '#user-zipCode' ).val();
    const del = $( '#user-delegation' );
    this.delegation = '';
    TunisiaGovAndDelg.getDelegFromGov(gov, del, zp, this.tunisiaData);
  }

  getDelegFromGovOnUp() {
    const gov = $( '#user-gouvernoratUp' ).val();
    const zp = $( '#user-zipCodeUp' ).val();
    const del = $( '#user-delegationUp' );
    this.delegationUp = '';
    TunisiaAdr.getDelegFromGov(gov, del, zp, this.tunisiaData);
  }

  getLatLngFromGov() {
    /* const gov = (<HTMLInputElement>document.getElementById('user-gouvernorat')).value;
  ...
  getGovAndDelegFromIndex() {
    const gov = $( '#user-gouvernorat' ).val();
    const del = $( '#user-delegation' ).val();
    const zp = $( '#user-zipCode' );
    const compoAdr = $( '#cityGlobalDest' );
    // TunisiaGovAndDelg.getGovAndDelegFromIndex(gov, del, zp, compoAdr);
    const vill = this.delegation + ', ' + gov;
    compoAdr.val(vill);
    const addr = (<HTMLInputElement>document.getElementById('user-cityGlobalDest')).value;
  ...
  getAdrFromIndex() {
    const gov = $( '#user-gouvernoratUp' ).val();
    const del = $( '#user-delegationUp' ).val();
    const zp = $( '#user-zipCodeUp' );
    const compoAdr = $( '#user-cityGlobalDestUp' );
    // TunisiaAdr.getAdrFromIndex(gov, del, zp, compoAdr);
    const vill = this.delegationUp + ', ' + gov;
    compoAdr.val(vill);
    const addr = (<HTMLInputElement>document.getElementById('user-cityGlobalDestUp')).value;
  ...
  showResult(result) {
    // (<HTMLInputElement>document.getElementById('user-latitude2')).value = result.geometry.location.lat();
    // (<HTMLInputElement>document.getElementById('user-longitude2')).value = result.geometry.location.lng();
    $( '#user-latGlobalDest' ).val(result.geometry.location.lat());
    $( '#user-lngGlobalDest' ).val(result.geometry.location.lng());
  }
  ...
  EditAddressDest() {
    ...
    if (titleBtn === 'Editer') {
      ...
      $('#user-newAdrDest').show();
      $('#user-btnShowHide').text('Annuler');
      $('#user-inputDestTrip').hide();
    } else {
      this.cityGlobalDestUp = this.cityAdrUp;
      this.latGlobalDestUp = this.latAdrUp;
      this.lngGlobalDestUp = this.lngAdrUp;
      $('#user-newAdrDest').hide();
      $('#user-inputDestTrip').show();
      $('#user-btnShowHide').text('Editer');
    }
  }


  EditAddressSource() {
    const titleBtn = $('#user-btnShowHide1').text();
    if (titleBtn === 'Editer') {
      ...
      $('#user-newAdrSource').show();
      $('#user-btnShowHide1').text('Annuler');
      $('#user-inputSourceTrip').hide();
    } else {
      this.cityGlobalSourceUp = this.cityAdrUp1;
      this.latGlobalSourceUp = this.latAdrUp1;
      this.lngGlobalSourceUp = this.lngAdrUp1;
      $('#user-newAdrSource').hide();
      $('#user-inputSourceTrip').show();
      $('#user-btnShowHide1').text('Editer');
    }
  }
  ...
  getvalue = (item: String, content: any) => {
    if (this.permiss === true ) {
      if (this.checkedTrips.length <= 0) {
        ...
        for (let i = 0; i < this.checkedTrips.length; i++) {
          if ((this.checkedTrips[i].statusTrip === 'Retour') || (this.checkedTrips[i].statusTrip === 'Retournee')) {
            this.checkedTrips[i].packageTrip.valPackage = 0;
            console.log('retour: ', this.checkedTrips[i].packageTrip.valPackage);
          }
        }
  ...
  generateExcelReportForClient() {
    ...
    for (let i = 0; i < this.checkedTrips.length; i++) {
      ...
      if ((jTemp.statusTrip === 'Retournee') || (jTemp.statusTrip === 'Retour')) {
        if (jTemp.isBilled === true) {
          jTemp.packageTrip.valPackage = 0;
          jTemp.statusTrip = 'Retournee';
        } else {
          jTemp.costTrip = 3;
          jTemp.packageTrip.valPackage = 0;
          jTemp.statusTrip = 'Retournee';
  ...
  getSelectedAdmin(admin: any) {
    if (admin != null) {
      this.selectedAdmin = admin.title;
      console.log('idSelectedadmin', this.selectedAdmin);
      const ind = this.Listadminauto.indexOf(this.selectedAdmin);
      console.log('iiiindadmin ', ind);
      this.adminFilterkey = this.Listadmin[ind].idAdmin;
      console.log('keyy ', this.adminFilterkey);
  ...
  getSelectedDriver2(driver: any) {
    const ind = this.Listdriverauto.indexOf(driver);
    const driverreturn = this.Listdriver[ind];
  ...
  BSFromServer(contentBSTelechargement) {
    ...
    this.tservice.getBS(this.actionA.idDriver, this.ListIdTrip).subscribe(data => {
      this.result = data['_body'];
      console.log(data['_body']);
      this.path1 = data['_body'];
      this.ListIdTrip = [];
      this.checkedTrips = [];
      this.spinner.hide();
      this.open(contentBSTelechargement);
      this.actionA = '';
      this.driverFilter = '';
    });
    // const data: any =  this.checkedTrips;

  }


  RCFromServer(contentRCTelechargement) {
    this.spinner.show();
    for (let i = 0; i < this.checkedTrips.length; i++) {
      this.ListIdTrip.push(this.checkedTrips[i].idTrip);
    }
    this.tservice.getRC(this.ListIdTrip).subscribe(data => {
      this.result = data['_body'];
      console.log(data['_body']);
      this.path2 = data['_body'];
      this.ListIdTrip = [];
      this.checkedTrips = [];
      this.spinner.hide();
      this.open(contentRCTelechargement);
    });
    // const data: any =  this.checkedTrips;
  ...
  BLFromServer(contentBLTelechargement) {
    this.spinner.show();
    for (let i = 0; i < this.checkedTrips.length; i++) {
      this.ListIdTrip.push(this.checkedTrips[i].idTrip);
    }
    this.tservice.getBL(this.ListIdTrip).subscribe(data => {
      this.result = data['_body'];
      console.log(data['_body']);
      this.path3 = data['_body'];
      this.ListIdTrip = [];
      this.checkedTrips = [];
      this.spinner.hide();
      this.open(contentBLTelechargement);
    });
    // const data: any =  this.checkedTrips;
  ...
  BRetourFromServer(contentBRetourTelechargement) {
    ...
    this.tservice.getBRetour(this.actionA.idDriver, this.ListIdTrip).subscribe(data => {
      this.result = data['_body'];
      console.log(data['_body']);
      this.path4 = data['_body'];
      this.ListIdTrip = [];
      this.checkedTrips = [];
      this.spinner.hide();
      this.open(contentBRetourTelechargement);
      this.actionA = '';
      this.driverFilter = '';
    });
    // const data: any =  this.checkedTrips;
  ...
  RapportLivFromServer(contentRapportTelechargement) {
    this.spinner.show();
    for (let i = 0; i < this.checkedTrips.length; i++) {
      this.ListIdTrip.push(this.checkedTrips[i].idTrip);
    }
    this.tservice.getRapport(this.ListIdTrip).subscribe(data => {
      this.result = data['_body'];
      console.log(data['_body']);
      this.path5 = data['_body'];
      this.ListIdTrip = [];
      this.checkedTrips = [];
      this.spinner.hide();
      this.open(contentRapportTelechargement);
    });
    // const data: any =  this.checkedTrips;
  ...
  Viderlalist() {
    ...
    this.checkedTrips = [];
    for (let i = 0; i < this.checkedTrips.length; i++) {
      this.checkedTrips[i].selected = false;
    }
    this.items = [];
    // console.log('liste aprés--- ', this.checkedTrips);

  }

  decocherListSelected(listSlected) {
    for (let i = 0; i < listSlected.length; i++) {
...
@Component({
  selector: 'user-dialog-overview-example-dialog',
  templateUrl: 'user-dialog-overview-example-dialog.html',
})
export class UserDialogOverviewExampleDialog {

  constructor(
    public dialogRef: MatDialogRef<UserDialogOverviewExampleDialog>,
