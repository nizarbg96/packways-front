...
export class AddressComponent implements OnInit {
  ...
  getAdresses() {
    ...
    this.tservice.getAdressByUser(this.id).subscribe(data => {
        ...
        const obj = Array.of(jo.data);
        this.jsonObj = obj[0];
        console.log('list adress', this.jsonObj);
...
addAdresse(model) {
  ...
  const a = $('#cityGlobalDest').val(); // il faut supprimer cityGlobalDest de partout
   if (this.labelAdresse === undefined) {
    ...
    this.snackBar.open('Veuillez remplir la gouvernorat', 'Fermer', {
      duration: 5000,
    });
    return;
  } else if (this.delegation === undefined) {
    this.snackBar.open('Veuillez remplir la ville', 'Fermer', {
      duration: 5000,
    });
    return;
  } else if (this.adresse === undefined) {
  ...
  // concatination d adresse
  this.cityGlobalDest = this.adresse + ' ' + this.gouvernorat + ' ' + this.delegation;
